...
-- Displays the grid data as a readable grid
...
-- Displays the grid data with possible numbers rather that dots
...
--returns a new list formed from the contents of the old lists grouped into groups of n
group :: Int -> [a] -> [[a]]
group _ [] = []
group n l
...
--inserts elements into their proper positions in the right diagonal of a 9x9 grid.
...
--inserts elements into their proper positions in the left diagonal of a 9x9 grid.
...
--takes a 9x10 grid, removes the top row (the diagonal elements) and replaces the diagonal elements of the 9x9 grid with these
--returning a 9x9 grid. For the right diagonal
...
--takes a 9x10 grid, removes the top row (the diagonal elements) and replaces the diagonal elements of the 9x9 grid with these
--returning a 9x9 grid. For the left diagonal
...
-- Transforms a Grid by changing subGrids to Rows and the other way around
...
pruneGrid' grid =
  ...
  >>= fmap insertBack1. traverse pruneCells . getDiagonal1   --our calls added here. one for each diagonal. 
  >>= fmap insertBack2. traverse pruneCells . getDiagonal2   --get diagonal places a new row of the diagonal on the top of the grid
                                                             --this is pruned and then the elements are inserted back into the grid
...
-- Checks if grid is solved by looking for any Possible cells.
isGridFilled :: Grid -> Bool
isGridFilled grid = null [ () | Possible _ <- concat grid ]



-- Checks if grid is valid by looking any duplicate fixed cells any empty Possible cell.
...
--When grid is settled, make a fixed cell by choosing a possibility then do backtracking search
...
-- Takes a sudoku grid and returns it’s solution if it exists
